...
from UltraMusic.helpers import admin_check
...
async def _mute(_, m: types.Message):
    ...
    if await db.muted(m.chat.id):
        try:
            return await m.reply_text(
                "<blockquote>⚠️ ᴛʜᴇ ꜱᴛʀᴇᴀᴍ ɪꜱ ᴀʟʀᴇᴀᴅʏ ᴍᴜᴛᴇᴅ.</blockquote>"
            )
        except (ChatSendPlainForbidden, ChatWriteForbidden):
            return
    ...
    try:
        await m.reply_text(
            f"<blockquote><b>🔇 ꜱᴛʀᴇᴀᴍ ᴍᴜᴛᴇᴅ ʙʏ</b> {m.from_user.mention}</blockquote>"
        )
    except (ChatSendPlainForbidden, ChatWriteForbidden):
